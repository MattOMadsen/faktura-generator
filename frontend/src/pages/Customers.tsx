...
export default function Customers() {
  ...
  const { data: customers = [], isLoading } = useQuery({
    queryKey: ['customers'],
    queryFn: () => fetch(`${API}/customers`, { headers: authHeaders(token) }).then((r) => r.json()),
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      fetch(`${API}/customers/${id}`, { method: 'DELETE', headers: authHeaders(token) }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['customers'] }),
  });

  const resetForm = () => {
    setForm({ name: '', email: '', address: '', cvr: '', ean: '', phone: '', notes: '' });
    setShowForm(false);
    setEditingId(null);
  };
                  ...
                  <tr key={c.id} className="border-b last:border-0 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            if (confirm('Slet kunde?')) deleteMutation.mutate(c.id);
                          }}
