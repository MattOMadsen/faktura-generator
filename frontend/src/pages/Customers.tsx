...
export default function Customers() {
  ...
  const { data: customers = [], isLoading } = useQuery({
    queryKey: ['customers'],
    queryFn: () => fetch(`${API_URL}/customers`, { headers: authHeaders(token) }).then((r) => r.json()),
  });

  const createMutation = useMutation({
    mutationFn: () => fetch(`${API_URL}/customers`, { method: 'POST', headers: authHeaders(token), body: JSON.stringify(form) }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['customers'] }); resetForm(); },
  });

  const updateMutation = useMutation({
    mutationFn: (id: number) => fetch(`${API_URL}/customers/${id}`, { method: 'PATCH', headers: authHeaders(token), body: JSON.stringify(form) }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['customers'] }); resetForm(); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => fetch(`${API_URL}/customers/${id}`, { method: 'DELETE', headers: authHeaders(token) }),
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowForm(!showForm)} className="btn-primary"><Plus size={18} /> {showForm ? 'Annuller' : 'Ny kunde'}</button>
      </div>
      ...
      {showForm && (
        <div className="card mb-6">
          ...
          <div className="flex gap-3 mt-4">
            <button onClick={() => editingId ? updateMutation.mutate(editingId) : createMutation.mutate()} className="btn-primary" disabled={createMutation.isPending || updateMutation.isPending}><Check size={16} /> {editingId ? 'Gem' : 'Opret'}</button>
      ...
      <div className="card overflow-hidden">
        {customers.length === 0 ? (
          <div className="text-center py-12 text-gray-400"><Users size={40} className="mx-auto mb-3 opacity-50" /><p>Du har ingen kunder endnu</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b"><th className="text-left py-3 px-4 font-medium text-gray-500">Navn</th><th className="text-left py-3 px-4 font-medium text-gray-500">E-mail</th><th className="text-left py-3 px-4 font-medium text-gray-500">Adresse</th><th className="text-left py-3 px-4 font-medium text-gray-500">CVR</th><th className="text-center py-3 px-4 font-medium text-gray-500">Handling</th></tr>
              </thead>
              <tbody>
                {customers.map((c: any) => (
