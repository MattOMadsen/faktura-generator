...
// Alle routes kræver login
router.use(authMiddleware);

// Hent alle fakturaer for bruger
...
router.post('/', async (req, res) => {
  const {
    ...
    customer_cvr, customer_ean, line_items, subtotal, vat_total,
    total, vat_rate, due_date, issue_date, payment_terms, notes,
  } = req.body;
  ...
  try {
    ...
    const countResult = await pool.query(
      "SELECT COUNT(*) FROM invoices WHERE invoice_number LIKE 'FAK-' || $1 || '-%' AND user_id = $2",
      [year, req.userId]
    );
    ...
    const result = await pool.query(
      `INSERT INTO invoices (
        invoice_number, user_id, customer_id, customer_name, customer_email,
      ...
      RETURNING *`,
      [
        invoiceNumber, req.userId, customer_id, customer_name, customer_email,
